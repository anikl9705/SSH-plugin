...
bash ./html-client-sdk/tools/vCenter\ plugin\ registration/prebuilt/extension-registration.sh	-action registerPlugin -remote  -url https://photon-machine/sdk \
  -username	$username -password $password -key com.sample.app.ssh -version 1.0.0 -pluginUrl $plug_url -serverThumbprint $sha1 \
  -c 'Sample' -n 'SSH Plug-in' -s 'This is a ssh sample plug-in'